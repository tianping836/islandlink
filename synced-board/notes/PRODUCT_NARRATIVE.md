...
不需要跨平台，就没有理由承担跨平台的代价。 Flutter 和 React Native 的价值是"写一次双端运行"。只
```
...
不需要跨平台，就没有理由承担跨平台的代价。 Flutter 和 React Native 的价值是"写一次双端运行"。只做 iOS 时，它们带来的性能折扣、原生能力受限
```

## Excerpt 3

```text
审、一段真实的协作。没有算法制造的关系，只有事件里长出来的网络。
